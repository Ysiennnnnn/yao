...
def main():
    # 读取数据
    a_data = read_json('/root/yao/retrieval/eval_data_use/health_claims_processed.json')  
    ...
    for a_item in a_data:
        if "id" not in a_item or "question" not in a_item:
            continue  # 跳过没有 'id' 或 'question' 键的条目
        ...
        for b_item in b_data:
            if "id" not in b_item or "instruction" not in b_item:
                continue  # 跳过没有 'id' 或 'instruction' 键的条目
    ...
    # 保存结果到c.json
    with open('/root/yao/retrieval/index_build_save/health_claims_processed_index.json', 'w', encoding='utf-8') as outfile:
